;(function(){
  ...
  */
    ...
    var Axis = function(opt){
      ...
      that.xAxisTextOffsetY = opt.xAxisTextOffsetY ? opt.xAxisTextOffsetY : '1.6em';
      that.yAxisTextOffsetX = opt.yAxisTextOffsetX ? opt.yAxisTextOffsetX : '1em';
      ...
      that.showYLine = opt.showYLine ? opt.showYLine : false;
      that.textAnchor = opt.textAnchor ? opt.textAnchor : 'middle';
    ...
    Axis.prototype = {
      ...
      initDom : function () {
        ...
        if(that.showYLine) {
          that.yLine = that.graph.select('g.y')
            .append('line')
            .attr('class', 'axis-line')
            .attr('x0', 0)
            .attr('y0', 0)
            .attr('x1', 0)
            .attr('y1', that.height - that.padding.top - that.padding.bottom)
            .style('opacity', 0);
        }
      ...
      initScale: function(){
        ...
        if(!that.horizontal){
          ...
          if(that.type == 'time'){
            that.xScale
              .range([0, that.width - that.padding.left]);
            that.xAxis.ticks(that.tickSize * 2).tickFormat(d3.time.format("%X"));
      ...
      update : function(x, y, keyNames, y1){
        ...
        if(!y) {
          that.yScale.domain(that.y).range([that.height - that.padding.top - that.padding.bottom, 0]);
        } else {
          that.yScale.domain(y).range([that.height - that.padding.top - that.padding.bottom, 0]); 
          that.y = y;
        }
        if(that.showYLine) {
          that.yLine
        ...
        that.yAxis.scale(that.yScale).tickFormat(d3.format(that.yformat)).ticks(that.tickSize);
        ...
        if(!that.horizontal){
          ...
          that.xAxisG.selectAll('text')
              ...
              .attr('transform', 'rotate(' + that.rotate + ')')
              .style('text-anchor', that.textAnchor)
          ...
          that.yAxisG.selectAll('text').attr('dx', '-' + that.yAxisTextOffsetX).style('font-size', '0.8em');
